...
namespace SurtidorADM.Views
{
    public partial class DashboardWindow : Window
    {
        ...
        private async void BtnAuditoria_Click(object sender, RoutedEventArgs e)
        {
            ...
            var auditoria = await _dashboardService.ObtenerAuditoriaFinancieraAsync(desde, hasta);
        ...
        private async void BtnBuscarPorFecha_Click(object sender, RoutedEventArgs e)
        {
            ...
            var debeCasheaVencido = await _dashboardService.CalcularDebeCasheaPorVencimientoAsync(desde, hasta);

            if (resultadoReporte != null && resultadoReporte.Ordenes.Count > 0)
            {
                dgResultados.ItemsSource = resultadoReporte.Ordenes;
                ActualizarMetricas(resultadoReporte.Ordenes, resultadoReporte.TotalVendido, resultadoReporte.TotalCaja, resultadoReporte.TotalFinanciado, debeCasheaVencido);
            }
            else
            {
                dgResultados.ItemsSource = null;
        ...
        private void ActualizarMetricas(List<dynamic> ordenes, decimal? totalVentaManual = null, decimal? totalCajaManual = null, decimal? totalFinanciadoManual = null, decimal? debeCasheaManual = null)
        {
            ...
            tbDebeCashea.Text = string.Format("{0:N3} $", debeCasheaManual ?? cuotasRestantesMonto);
        }
        ...
        private async void BtnVentas_Click(object sender, RoutedEventArgs e)
        {
            ...
            DateTime? desde = dpDesde.SelectedDate;
            DateTime? hasta = dpHasta.SelectedDate;
            dgResultados.ItemsSource = await _dashboardService.ObtenerResumenVentasAsync(desde, hasta);
        }
        ...
        private async void BtnConseguidas_Click(object sender, RoutedEventArgs e)
        {
            ...
            DateTime? desde = dpDesde.SelectedDate;
            DateTime? hasta = dpHasta.SelectedDate;
            var data = await _dashboardService.ObtenerEstadoConciliacionAsync(desde, hasta);
        ...
        private void BtnBancoReporte_Click(object sender, RoutedEventArgs e)
        {
            if (dpDesde.SelectedDate == null || dpHasta.SelectedDate == null)
            {
                MessageBox.Show("Por favor, seleccione ambas fechas.");
            ...
            var bancoWindow = new BancoWindow();
            bancoWindow.dpDesde.SelectedDate = dpDesde.SelectedDate;
            bancoWindow.dpHasta.SelectedDate = dpHasta.SelectedDate;
            bancoWindow.Show();
            bancoWindow.EjecutarBusqueda();
        }

        private void BtnReportes_Click(object sender, RoutedEventArgs e)
        {
            var reportesWindow = new ReportesWindow();
            reportesWindow.dpDesde.SelectedDate = dpDesde.SelectedDate;
            reportesWindow.dpHasta.SelectedDate = dpHasta.SelectedDate;
            reportesWindow.Show();
        }
    }
}